...
### Added
- **左栏「工作区」重新定义为多项目容器（V21）**：左栏顶层从「项目平铺」变成 **工作区 → 项目 → 目录行**（项目下的主目录 / worktree 展示口径不变）。一个工作区可以装多个项目（如「全栈电商」= frontend + backend）：组头点击切到该工作区的终端视图（右栏显示组内全部目录的终端），组头 hover 的 `＋` 在组内首个项目的主目录开终端、`PenLine` 打开编辑对话框（名字 + 成员勾选 + 删除工作区）。没有自定义工作区时项目仍平铺（单项目用户无感）；有自定义组时其余项目收进「未分组」区；移出成员 = 回归未分组，**不删项目、不动文件**。入口：左栏标题行的 `Layers` 按钮；对话框里的「添加本地目录」可直接选本地文件夹为项目（走与左栏「添加项目」同一实现 `pickAndAddProject`），新项目立刻进列表并自动勾选为成员——不必先退出对话框去左栏加项目；失败就地显示（模态遮着侧栏，错误条放外面会看不见）。
- **工作区内多项目自动协作（V21 核心）**：工作区成员 ≥ 2 时，在其中任何目录开的终端会自动把**其余成员项目的主目录**作为附加根挂进 omp（`--add-dir`；实测跨项目 write / read 与各项目自己的 AGENTS.md 规则都生效），并注入一段「工作区拓扑」说明（`--append-system-prompt`：成员清单 + 路径 + 当前主目录标记 + 跨项目同时验证的约定 + 需要并行时用 task 子代理）——给一个需求，前端与后端可以同时改、各自跑验证。终端标签的悬停提示显示该终端**实际挂上**的协作根（spawn 时写回 store 的快照）。单成员工作区与未分组项目不追加任何参数（与 V20 行为一致）。

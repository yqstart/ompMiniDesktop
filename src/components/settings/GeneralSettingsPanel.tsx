        ...
        <div className="mt-1.5 mb-0.5 space-y-0.5 rounded-md border border-border bg-background p-1">
          {optionsFor(spec, value).map((opt) => (
            <button
              ...
              className={`block w-full cursor-pointer truncate rounded px-2 py-1 text-left text-[13px] transition-colors duration-100 hover:bg-hover ${
...
function Switch({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onToggle}
      className={`relative h-[18px] w-8 shrink-0 cursor-pointer rounded-full transition-colors duration-100 disabled:opacity-40 ${
        on ? "bg-accent" : "bg-border"
      }`}
    >
      <span
        className={`absolute top-[2px] h-[14px] w-[14px] rounded-full bg-surface transition-transform duration-100 ${

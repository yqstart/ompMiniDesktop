...
export function FallbackChainsSection({
  ...
  const write = async (fn: () => Promise<FallbackChainsInfo>, failMsg: string) => {
    setErr(null);
    setWriting(true);
    try {
      onSaved(await fn());
        ...
        <>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <button
              ...
              className={`shrink-0 cursor-pointer rounded-md border px-2.5 py-1 text-[13px] transition-colors duration-100 disabled:opacity-50 ${
                enabled
                ...
                {(
                  ...
                  return (
                    <button
                      ...
                      className={`cursor-pointer rounded-sm px-2 py-0.5 text-[12px] transition-colors duration-100 disabled:opacity-50 ${
                        selected ? "bg-active text-foreground" : "text-muted hover:bg-hover hover:text-foreground"
                      }`}
                    >
                      {label}
                    </button>
...
function ChainEditor({
  ...
  const canSave = key.trim() !== "" && list.length > 0;
  ...
  const levelsFor = (sel: string): string[] => {
    const m = models.find((x) => x.selector === splitSelector(sel).base);
          ...
          >
            {(
              ...
              <button
                ...
                className={`cursor-pointer rounded-sm px-2 py-0.5 text-[12px] transition-colors duration-100 ${
                  kind === k ? "bg-active text-foreground" : "text-muted hover:bg-hover hover:text-foreground"
                }`}
              >
                {label}
              </button>
        ...
        <div className="mt-1.5">
              ...
              {roles.map((r) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={key === r}
                    className={`cursor-pointer rounded-md border px-2 py-0.5 text-[13px] transition-colors duration-100 disabled:cursor-default disabled:opacity-40 ${
                      key === r ? "border-accent/50 bg-active text-foreground" : "border-border text-muted hover:bg-hover hover:text-foreground"
                    }`}
                  >
                    {roleLabel(r, t)}
                  </button>
              ...
              {providers.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`cursor-pointer rounded-md border px-2 py-0.5 font-mono text-xs transition-colors duration-100 disabled:cursor-default disabled:opacity-40 ${
                      key === value ? "border-accent/50 bg-active text-foreground" : "border-border text-muted hover:bg-hover hover:text-foreground"
                    }`}
                  >
                    {value}
                  </button>
          ...
          {kind === "model" && (
            <div className="rounded-md border border-border bg-surface p-2">
              <ModelPickList models={models} onPick={(m) => setKey(m.selector)} />
            </div>
          )}
        </div>
      )}

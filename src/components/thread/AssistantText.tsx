...
export function AssistantText({ text, complete }: { text: string; complete: boolean }) {
 ...
 return (
  <div className="md-body max-w-full overflow-x-auto">
   ...
   {pendingUrl && (
    <span className="mx-1 inline-flex items-center gap-1 rounded-md border border-border bg-elevated px-1.5 py-0.5 text-[13px]">
   ...
   {openFence && (
    <div className="mt-1.5 space-y-1.5 rounded-md bg-code p-3" aria-label={t.codeBlockGenerating} role="status">
...
function CodeBlock({ children, ...props }: React.ComponentProps<"pre">) {
 ...
 return (
  <div className="group/code relative">
   <pre {...props} className="overflow-x-auto rounded-md bg-code p-3 font-mono text-xs leading-5">
    {children}
   </pre>
   <button
    onClick={() => void copy()}
    className="absolute top-1.5 right-1.5 flex cursor-pointer items-center gap-1 rounded-md border border-border bg-elevated px-1.5 py-0.5 text-[11px] text-muted opacity-0 transition-opacity duration-150 group-hover/code:opacity-100 focus-visible:opacity-100"
    aria-label={t.copy}
    title={t.copy}
   >

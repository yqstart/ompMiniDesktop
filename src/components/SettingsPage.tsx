...
/** 设置页分页签；顺序即界面顺序。 */
type SettingsTab = "general" | "models" | "memories" | "usage" | "providerUsage" | "archived";
const TAB_ICONS = { general: Sliders, models: Key, memories: Notebook, usage: ChartBar, providerUsage: Gauge, archived: Archive };
const CONFIG_TABS: SettingsTab[] = ["general", "models"];
const DATA_TABS: SettingsTab[] = ["memories", "usage", "providerUsage", "archived"];
 ...
 return (
  ...
  >
   ...
   <nav aria-label={t.title} className="flex w-11 shrink-0 flex-col border-r border-border-soft pr-2 @min-[640px]/settings:w-40 @min-[640px]/settings:pr-3">
    ...
    >
     {[CONFIG_TABS, DATA_TABS].map((group, gi) => (
      <div key={gi} className={`flex flex-col gap-1 ${gi > 0 ? "mt-3 border-t border-border-soft pt-3" : ""}`}>
       {group.map((k) => {
     ...
     <>
      <div className="shrink-0">
       ...
       <div className="mt-2 flex flex-wrap gap-1.5">
        {[["settings-general-options", t.settingsGeneralJumpOptions], ["settings-general-diagnostics", t.settingsGeneralJumpDiagnostics], ["settings-general-update", t.settingsGeneralJumpUpdate]].map(([id, label]) => (
         <button
          key={id}
          onClick={() => {
           const el = panelRef.current?.querySelector<HTMLElement>(`#${id}`);
      ...
      <div id="settings-general-options" className="scroll-mt-2">
       <GeneralSettingsPanel />
      </div>

      <section id="settings-general-diagnostics" aria-label={t.diagSection} className="scroll-mt-2 shrink-0 rounded-lg border border-border-soft bg-surface p-4 @min-[480px]/panel:p-5">
       <div className="flex flex-wrap items-center gap-2">
        <h2 tabIndex={-1} className="text-sm font-semibold outline-none">{t.diagSection}</h2>
      ...
      <section id="settings-general-update" aria-label={t.updateSection} className="scroll-mt-2 shrink-0 rounded-lg border border-border-soft bg-surface p-4 @min-[480px]/panel:p-5">
       <div className="flex flex-wrap items-center gap-2">
        <h2 tabIndex={-1} className="text-sm font-semibold outline-none">{t.updateSection}</h2>

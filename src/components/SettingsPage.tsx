...
import { LOCALES, SETTINGS_TEXT, type Locale } from "../lib/locale";
...
export function SettingsPage() {
  ...
  const fill = (tpl: string, v: string) => tpl.replace("{v}", v);
  const updateHint =
    update.status === "latest"
      ? fill(t.updateLatest, update.current)
      : update.status === "available"
        ? fill(t.updateAvailable, update.version)
        : update.status === "downloading"
          ? t.updateDownloading
          : update.status === "ready"
            ? fill(t.updateReady, update.version)
            : update.status === "error"
              ? fill(t.updateError, update.message)
              : null;

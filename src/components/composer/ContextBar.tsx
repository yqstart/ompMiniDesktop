...
export function ContextBar() {
 ...
 return (
  <div className="mx-auto flex max-w-3xl flex-wrap items-center gap-x-1 gap-y-0.5 px-3.5 pb-1.5 text-[13px]">
   <ProjectPicker project={project} />
   {git?.isRepo ? (
    <BranchPicker git={git} loading={loading} onReload={reload} />
   ) : git && cwd ? (
    <span className="px-2 py-1 text-faint" title={git.error ?? undefined}>
     {t.notGitRepo}
    </span>
   ) : null}
   <UsageLimits />

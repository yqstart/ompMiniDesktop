...
src/
  ...
  components/sidebar/
    WorkspaceSidebar.tsx   # 左栏：添加项目 + 新建工作区 + 工作区/项目树 + 底部「设置/语言/皮肤」行（更新小点在设置入口上）
    WorkspaceGroupSection.tsx  # 工作区段（V21）：组头（选中/折叠/＋终端/编辑） + 成员项目（null = 「未分组」区）
    WorkspaceGroupDialog.tsx   # 工作区新建 / 编辑对话框（V21）：名字 + 成员项目勾选 + 删除组（成员回归未分组）
    ProjectGroup.tsx       # 项目组：折叠头（会话入口 / 移除项目，文件夹图标标活动项目）+ 目录行（worktree 只读展示）
...
- **右栏只显示当前选中范围的终端**（V21；`lib/terminalScope.ts` 的 `terminalsInScope` + `lib/workspaceGroups.ts` 的 `selectionScopePaths`）：`selection` = `{kind:"group", id}`（工作区视图，`id:null` = 未分组；范围 = 组内全部项目的全部目录）或 `{kind:"checkout", path}`（目录视图）；`null`（一个可用目录都没有）= 不过滤。别的范围的终端照常跑、只是不在这个视图里（隐藏 = CSS 显隐、不 kill；左栏目录行的终端数徽章与 `⌘⇧K` 快速切换是它们的入口，跳过去左栏跟着切）。不变式：`activeTerminalId` 要么 null、要么落在范围里——`openTerminal`（组视图里开终端保持组视图）、`focusTerminal`（范围外才切到它的目录视图）、`closeTerminal`（范围内相邻收敛）、`loadCheckouts`（选中失效回退 + 激活终端收敛）四处维持。

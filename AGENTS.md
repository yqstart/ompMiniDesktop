...
src/
  ...
  components/composer/     # Composer（一体式输入框 + 工具行 + 图片附件：粘贴/拖拽/选文件，发送随 prompt.images；流式中 Enter 排队 + ⌘/Ctrl+Enter 转向 + `@` 路径补全 + 图文混贴保留文字 + 乐观回显 + 草稿持久化；`/` 开头照旧作为本地命令直发，但**不再弹命令列表**）、CapabilityMenu（会话能力开关：顾问 / 电脑可切，计划 / 目标仅 TUI 说明）、ContextBar（输入框上方一行：项目 + git 分支 + 用量限额）、ContextMeter（上下文容量：模型选择器左侧的容量环 + 点开的分项面板，数据来自 `get_context_breakdown`）、UsageLimits（用量限额入口：当前会话供应商的配额进度条 + 展开的窗口明细，数据来自 `omp usage --json`；一份配额都拿不到时自身不渲染）
...
- **会话能力开关（输入框工具行 `CapabilityMenu`）**：omp 内建斜杠命令分两类——**同时有 `handle`（ACP/RPC）与 `handleTui` 的才能在 RPC 里跑**（`computer` / `advisor` 属于这类，壳侧按普通 `prompt` 下发本地命令，无 agent turn、不计 token）；**只有 `handleTui` 的**（`plan` / `goal` / `vibe` / `loop` / `queue`…）在 RPC 侧**没有分发入口**——把 `/plan` 当 prompt 发出去会真的开一个 agent turn 把文字喂给模型，所以壳侧**不许**靠发 `/plan` 切模式，界面如实标「仅 TUI」。开关状态**只能从 omp 自己的 `<cmd> status` 输出里读**（`get_state` 没有这两个字段）：`/computer status` → `Computer use: enabled · prelude: active · configured: …`，`/advisor status` → `Advisor is enabled (provider/model). …` / `Advisor is disabled.`；开关自己的回执不带状态，所以切完要再探一次。解析与命令拼装的唯一实现在 `src/lib/capabilities.ts`（纯函数 + 单测），**拿不到状态就显示「未查询」，不猜、不摆假值**。

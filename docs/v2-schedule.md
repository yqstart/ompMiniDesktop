...
| M7 会话发现与规模 | 扫描分页（>500 个 jsonl 不再截断）；左栏搜索覆盖会话内容 | `docs/v1-schedule.md` §3.3「分页/后台补全留 V2」 | 分页已完成，搜索待做 |
...
- 左栏搜索现在只匹配标题/备注；要扩到「命中内容片段」需按需读 jsonl 中段（禁止全量预读），并在结果里给出高亮片段与命中位置。

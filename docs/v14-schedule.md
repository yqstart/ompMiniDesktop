...
- 不传 `-c` 自由文本附加说明（V14 当时不做；**2026-09-23 的「提交信息语言」档已落地**——那只是固定档位对应的要求文本，仍不给自由输入，见 §7）。
...
- `-c/--context` 可传额外要求（实测中文：正文中文、**摘要仍必须英文过去式动词开头**——上游校验器硬约束，
  不承诺全中文）。V14 增补（§7）用它实现「提交信息语言」三档。
...
- 浮层内可选「附加说明」（`omp commit -c <text>`）的**自由输入框**（语言三档已按项目落地，见 §7；
  自由文本仍待用户确认再开工）。
...
## 7. 增补（2026-09-23）：提交信息语言（系统默认 / 中文 / 英文，按项目记忆）

**口径**：`omp commit` 生成什么语言的提交信息，在提交浮层里选三档，并可「记住选择」——
记忆粒度是**项目**（不是全局，也不是单个工作区）：同一项目的主目录与全部 worktree 共用一份，
项目之间互不影响。

|档位|传给上游的行为|
|---|---|
|系统默认|不传 `--context`，跟从 omp 自身行为（今天的默认是英文）|
|中文|`--context="请用简体中文撰写提交信息：正文与摘要都用简体中文；摘要首词按校验器要求保留英文过去式动词。"`|
|英文|`--context="请用英文撰写提交信息：摘要与正文都用英文，摘要首词为英文过去式动词。"`|

**上游实测（omp 18.2.10，隔离临时仓库）**：`omp commit -c "<中文要求>"` 有效——摘要主体与正文可中文
（实测产出 `chore: added a.txt 与 b.txt 文件`、正文中文），但**摘要首词必须是英文过去式动词**：
校验器依次拒绝「添加」「新增」「已添加」，只有 `added …` 通过（模型自己在日志里点明这条约束）。
因此「中文」档在浮层里常驻一行小字说明首词约束——**壳侧不代偿、不改写上游产物**。

**存储**：`localStorage["omp.commitLang.v1"] = { "<projectId>": "system" | "zh" | "en" }`，
只落**已记住**的项目（`src/lib/commitLang.ts`）；未记住的选择活在 store 的内存态
（`commitLangPrefs`），只在本次运行期间生效。键是**项目 id**：重定位（relocate）不丢记忆；
移除项目后再添加同一目录会拿到新 id，旧条目只残留不生效（无害，不主动清理）。

**数据流**：浮层控件 → `useApp.setCommitLangPref(projectId, pref)` → 任务发起时
`lib/commitTasks.ts` 的 `contextArgFor(cwd)`（按工作区归属查 projectId）→
`api.startCommitPush(cwd, context, channel)` / `api.pushCommits(cwd, context, channel)` →
Rust `commit_args` 拼 `--context=<值>`（等号形式，值里的空格与中文不经 shell）。
**两段都传**：推送段若遇到等待期间的新改动会先提交，语言要求跟着走。

**交互**：
- 控件在浮层底部（「提交信息语言」三档 + 「记住选择（本项目）」开关），与左栏两个三档控件同款
  （`role="radiogroup"` + `role="radio"`、左右方向键组内循环、选中 `bg-active`；开关复用设置页的 `Switch`）；
- **运行中锁住**（本次任务的参数已定，改了也只影响下次）：控件禁用 + 一行
  「任务运行中不可修改，结束后可改（下次提交生效）」；`failed` / `committed` 等终态当场可改，
  随后的「重试 / 推送 / 下次任务」按新选择走；
- 「记住选择」= 写盘；取消 = 删键（内存态保留，本次运行仍生效）。语言名是**自称**
  （`简体中文` / `English`，复用 `LOCALE_NAMES`，不随界面语言翻译；只有「系统默认」进字典）。

**验证**：Rust `commit_args` 单测（None / 空白 → 不传；`--push` 与 `--context=` 的组合与顺序）；
vitest `commitLang.test.ts`（三档归一、坏数据丢弃、save / load 往返、`system` → null、
中文 / 英文要求互不相同）与 `CommitTaskPanel.test.tsx`（默认档选中、选中文只进内存、
勾记住后按项目写盘、worktree 共享同一份、取消记住删键、运行中禁用）；
真实仓库慢测试的第二段用 `--push --context=` 组合跑真实 omp（参数拼错即失败）；
界面核对（静态构建 + IPC mock，深浅两套皮肤）：三种态（运行中禁用 / committed / pushed）、
点中文 → 内存、点记住 → `{"p1":"zh"}` 落盘、worktree 浮层共享同一档、
`push_commits` 收到的 `context` 正是中文要求文本（端到端传递）。

远端真实调用另有一次手工实测：临时仓库 `omp commit -c "<中文要求>"`（omp 18.2.10）产出
`chore: added a.txt 与 b.txt 文件` + 中文正文，退出码 0——摘要首词被校验器锁在英文过去式动词。

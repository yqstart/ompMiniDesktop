...
## 8. 现有功能深度打磨（2026-09-17）

不新增模型管理范围，四区块顺序与「星标只影响本应用候选」保持不变。

### 8.1 配置安全与错误恢复

- 修复供应商撞名兜底仍可能覆盖原块的问题；新建撞名、改名撞名、编辑目标消失、配置结构异常及覆盖块写入均明确拒绝。
- 模型条目记录读取时的 `originalIndex`，按来源节点保留 cost / tokenizer / compat；改模型 id、删除前一行后改名不丢隐藏字段，删除后新建同名模型不继承被删除项。
- 只改实际变动字段，保留未改动的 `auth: oauth`、显式 `reasoning: false`、未知输入模态及注释。保真指字段语义与注释内容保留；yaml 序列化可能将集合行尾注释移动到下一行，不宣称所有合法输入逐字节不变。
- 名称、HTTP(S) 地址、重复模型 id、正整数限制给出字段级提示；供应商改名明确提示旧引用不会自动迁移。编辑文本/hash 固定为打开表单时的快照，保存失败保留草稿并聚焦错误；保存期间整表禁用、禁止 Esc / 遮罩 / × 误关。
- 后端预校验结束后再次比对原内容和生效文件路径，避免耗时校验期间外部编辑被旧草稿覆盖。原备份与原子写规则保留。
- `shared/api.ts` 统一归一 Rust 的结构化错误及字符串错误，界面可直接显示后端原因与恢复提示，不再只显示笼统的「操作失败」。

### 8.2 模型挑选、角色与失败转移

- 登录状态先订阅再恢复快照，启动失败可重试；启动中关闭会等待启动结果后取消，不遗留登录。后端每次 spawn 分配独立序号，旧进程的 URL / 输出 / 完成事件及清槽不能污染同一家供应商的新登录；序号只在 Rust 内部使用，IPC 形状不变。
- 筛选态显示匹配数，「全选匹配项 / 清空匹配项」明确操作范围；全部已选或全部未选时禁用对应无效动作。
- 模型选择器同时展示名称/id、当前选中态和上下文窗口；移除无说明的思考档数量数字。
- 候选依然按星标收窄，当前模型的能力却从完整目录读取：未星标的已配置模型不再被错误地当作支持全部思考档。
- 角色加载只接受最新请求，旧读请求不得覆盖新读取/保存结果；角色保存互斥，避免整表回读乱序。
- 新建失败转移链排除已有模型键并在保存前兜底检查，不再覆盖原链。修改启用开关/回归策略保留草稿；保存中禁止编辑；排序/删除目标收起原下标的档位选择，避免改错另一目标。

### 8.3 共用交互

- 设置竖向导航支持上下方向键、Home/End；隐藏设置用 inert 隔离焦点，返回恢复焦点，不卸载终端。
- 三种共用模态统一 Tab 圈定、焦点恢复与最上层 Esc；内部下拉先处理 Esc，嵌套删除确认不会连带关闭父弹窗。
- 会话删除确认后立即退出确认态，进行中防重复操作；下拉重渲染不抢回首项焦点。

### 8.4 验证结果

- `pnpm check`：类型检查、lint、97 项前端测试、43 个 IPC 命令双向契约通过；`pnpm build` 通过。构建仍提示主包超过 500KB，本轮未以调高阈值隐藏警告。
- `cargo test --manifest-path src-tauri/Cargo.toml`：166 项通过，5 项既有慢测试跳过。新增隔离假 omp 测试证实预校验期间内容修改/文件优先级切换拒写；本地 shell 回环确认旧登录清理不破坏新登录输入；旧状态更新被拒。

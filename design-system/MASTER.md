...
- `CapabilityMenu`（`src/components/composer/CapabilityMenu.tsx`）= **omp 会话级能力开关**（输入框工具行、紧跟 `PermissionBadge` 右侧（浮层**左对齐**触发按钮——它在工具行左半边；工具行右半边的选择器才用右对齐，理由与 §4 的弹层一致：面板宽 320px，右对齐会往左冲出窗口把行标签裁掉），向上弹）：收起态 = `Cpu` 图标 + mono 计数（`1/2`；一项状态都没读到显示 `?`——「未知」与「全关」不是一回事）；展开态四行：
  - **顾问（advisor）/ 电脑（computer）** = 真开关（`role="switch"`，胶囊 20×36，开 `bg-accent/80`）。状态**只认 omp 自己 `<cmd> status` 的输出**（打开面板时探一次，60s 内不重复；切完隔一拍再探一次确认），拿不到就显示「未查询」；行下细节直接透传 omp 的话（顾问接手的是哪个模型、computer 的 prelude 状态）。
  - **计划（plan）/ 目标（goal）** = **只读说明行**（`仅 TUI` 角标 + 一句原因）：omp 18.2.1 只把这两个命令实现在终端 TUI 里，RPC 没有分发入口，壳侧切不了——**不摆假开关**。
  - 开关动作走 `run_slash`（本地命令，无 agent turn），omp 的回执作为一行命令输出落在会话里；会话运行中（`running`）禁用并说明原因。
  - 面板底部一行 11px `faint` 写明口径（哪些壳侧能切、为什么另两项不能）。状态解析与命令拼装收在 `src/lib/capabilities.ts`（纯函数 + 单测）。

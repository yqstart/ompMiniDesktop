...
- `CapabilityMenu`（`src/components/composer/CapabilityMenu.tsx`）= **omp 会话级能力开关**（输入框工具行、紧跟 `PermissionBadge` 右侧，向上弹）：收起态 = `Cpu` 图标 + mono 计数（`1/2`；一项状态都没读到显示 `?`——「未知」与「全关」不是一回事）；展开态四行：

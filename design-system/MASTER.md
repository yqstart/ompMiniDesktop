...
- 工作区段（V21）：组头 = 32px 行（chevron + 16px `Layers` 图标 + 组名 + 成员计数 + hover 槽位的 `＋` 与 `PenLine`），点击整行 = 选中该工作区（`bg-active` 填充 + 左侧 2px `accent` 竖条，与树行选中态同款），折叠箭头独立可点；下方成员项目整体缩进一级（左引导线）。「未分组」区是同一个组件的 `group = null` 形态：不可编辑 / 不可删，只在存在自定义工作区（且有未分组项目）时出现。
- 项目组：36px 折叠头（chevron + 24px 文件夹图标底 + 项目名）与 hover/focus 操作槽（会话、移除项目）；**文件夹图标即「当前打开的项目」**——项目落在右栏选中范围里时用 `accent`，否则 `muted`，缺失目录优先 `warn`。「移除项目」走 `ConfirmDialog`（解绑 + 会话标记归档，不删文件）；缺失目录给 warning 修复条。
- 目录行（V21 前叫「工作区行」）：32px 行，`DiagramTree` + 等宽分支名 + worktree 徽章；选中左线、填充与加粗。主目录图标 accent，普通 worktree faint；缺失目录禁用。从属关系用弱引导线表达。worktree **只读展示**（V20：壳侧不创建 / 不删除，行上没有删除按钮）。
- 右侧工作面：桌面端外沿 8px 留白、16px 圆角与轻描边；终端本身无额外 padding。48px 常驻标签栏（即便没有任何标签也保留工作区标题与新建入口）——**终端标签按左栏选中范围过滤**（工作区视图 = 组内全部目录的终端，目录视图 = 该目录；别的范围的终端照常跑、不在这个视图里），激活标签 surface 填充与细边框；窄窗左端显示项目抽屉按钮。标题截断、关闭入口和常驻 `＋` 行为不变；标签溢出时栏内横向滚动、不显示滚动条（全 app 口径见 §7；垂直滚轮映射为横向）；终端和设置面板只切显隐，不卸载。
...
- `WorkspaceSidebar`（`src/components/sidebar/WorkspaceSidebar.tsx`）= 左栏主容器：红绿灯占位 +「添加项目」/「新建工作区」（`Layers`，V21）+ 工作区段 / 项目组列表 + 底部「设置」行（`LanguageToggle` / `ThemeToggle` / 更新小点）。添加项目失败等错误用内联 danger 条 + 关闭按钮，不造第三种提示样式。
- `WorkspaceGroupSection`（`src/components/sidebar/WorkspaceGroupSection.tsx`）= 工作区段（V21）：组头 + 成员 `ProjectGroup` 列表；`group = null` = 「未分组」区。组头 hover 的 `＋` = 在组内首个项目的主目录开终端（并先选中该组——新终端保持组视图），`PenLine` = 打开 `WorkspaceGroupDialog`；`aria-current` 标出被选中的组。
- `WorkspaceGroupDialog`（`src/components/sidebar/WorkspaceGroupDialog.tsx`）= 工作区新建 / 编辑对话框（V21，`DialogShell` 壳）：名字输入 + 成员项目勾选列表（已在别的组里的行尾标出组名）+ 区头右侧「添加本地目录」按钮（`FolderPlus`，走 `pickAndAddProject`：新项目立刻进列表并自动勾选为成员）+（编辑时）「删除工作区」走 `ConfirmDialog` 二次确认（成员回归未分组，不删项目 / 文件）。失败就地显示 danger 条（模态遮着侧栏，错误必须在这一层可见）。
...
- `SessionPopup`（`src/components/sidebar/SessionPopup.tsx`）= 项目会话弹窗（`fixed inset-0 z-30` 遮罩 + `max-w-lg` 卡，`max-h-[70vh]`）：标题（`项目名 · 会话` + mono 路径 + ×）→ 搜索框 + 元信息行（已加载 / 匹配 / 扫描窗口）→ 会话行（标题 + 时间 + 归档角标；行点击 = 新终端 `omp --resume`；hover 槽位 = 归档 / 恢复 + 删除）→ 常驻底部操作条（**不勾选**：「全部归档（N）」「全部删除（N）」，作用于**当前过滤结果**；全部归档只数未归档的那些、无适用项禁用）。删除走 `ConfirmDialog`（单条标题 = 会话名，全部删除 = 「删除全部 N 个会话？」）；挂载方用 `key={project.id}` 保证换项目即重挂载。
...
- `CommitTaskPanel`（`src/components/git/CommitTaskPanel.tsx`）= 提交 / 推送浮层（V19，App 常驻挂载、`activeCommitCwd` 控制显隐）：标题（提交 · 项目 · 分支）+ 阶段徽章（`Loader` 转 / `CheckCircle` / `AlertTriangle` / `X`）+ **轨道两档分段控件**（快速 / 完整（含 CHANGELOG））→ 快速轨：**文件勾选列表**（`role="checkbox"` 自绘方框 + `Check` + 状态 chip + mono 路径 + `+N −M`，「全选 / 全不选」在区头）+ **可编辑提交信息**（`textarea`，生成中 `readOnly` 并追加流式输出）+ 说明行；完整轨：流式日志（mono、后端已去 ANSI、自动滚底、上限 1000 行）+ 一行提示（会维护 CHANGELOG.md）→ 两轨共有：提交结果列表 + 错误与 hint + **提交信息语言**（三档分段控件：系统默认 / 简体中文 / English，与左栏两个控件同款 `role="radiogroup"` + `role="radio"` + 左右方向键；右侧「记住选择（本项目）」用共享 `Switch`；**按项目记忆** localStorage `omp.commitLang.v1`，运行中整组禁用并给一行说明）→ 底部按钮：运行中 = 取消 / 后台运行；快速轨 = 生成（重新生成）/ 提交 / 提交并推送 / 关闭（没有改动但有待推送提交时给「推送」）；完整轨 = 完整提交 / 推送（本次已产出提交时）/ 关闭。**消息为空时「提交」「提交并推送」禁用**（title 说明）。
